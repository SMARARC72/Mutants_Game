...
## The oracle CEILING stat block for a creature_instance via its species row. Returns the
## StatEngine.stat_block dict { "stats": Dictionary, "hp": int, "bst": int }, or {} if the species
## id is missing from the catalog (the caller shows a content-gap placeholder).
