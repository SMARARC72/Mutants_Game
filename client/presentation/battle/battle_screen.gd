...
func _process(_delta: float) -> void:
	...
	if router != null and int(router.call("depth")) > 0:
		return  # W11: while the result card is up it owns EVERY press — the first skips the name reveal,
...
## W17: the capture card's soul-page affordance (glue lives in CaptureCard — line cap).
func _show_dossier_button() -> void:
	CaptureCardScript.add_dossier_button(self, _game, _result, _root_box)
