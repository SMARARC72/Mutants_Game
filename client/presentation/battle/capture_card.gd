...
## W17 glue target: the soul-page pushed above an ended battle ("Read its Dossier").
...
static func add_dossier_button(
	screen: Control, game: Node, result: Dictionary, root_box: Control
) -> void:
	var caught: Dictionary = result.get("caught", {})
	if caught.is_empty() or screen.get_node_or_null("/root/UiRouter") == null:
		return
	var btn := Button.new()
	...
	btn.pressed.connect(
		func() -> void:
			...
			if page != null:
				...
				page.call("show_creature_dict", result.get("caught", {}))
	)
	if root_box != null and is_instance_valid(root_box):
		root_box.add_child(btn)
