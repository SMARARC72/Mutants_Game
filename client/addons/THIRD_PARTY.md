...
| **Beehave** | tag `v2.9.2` (`addons/beehave`) | MIT | https://github.com/bitbrain/beehave | **Not yet wired** — vendored ahead of the Realization plan's Wave 13 (ambient critters + NPC idle-wander/face-player trees). Nothing in-game runs a `BeehaveTree` today; battle AI is the self-contained `application/ai` kernel (see below). The editor plugin + its two debug autoloads (`BeehaveGlobalMetrics`/`BeehaveGlobalDebugger`) are DISABLED until first real use, because the plugin re-registers those autoloads on every editor load (W2 hygiene, red-team C15) — W13 re-enables the plugin when it wires the first tree. |
