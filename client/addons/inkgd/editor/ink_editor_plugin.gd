...
func _remove_autoloads():
	# Mutants_Game compat patch (Cluster 3, ADR-017): inkgd 0.6.0 (godot4 branch) no
	# longer auto-registers the "__InkRuntime" autoload, so removing it on _exit_tree
	# raises "nonexistent project setting" during headless import teardown. Guard it.
	# See client/addons/THIRD_PARTY.md > Local modifications.
	if ProjectSettings.has_setting("autoload/__InkRuntime"):
