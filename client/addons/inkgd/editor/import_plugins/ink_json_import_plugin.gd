...
func _get_recognized_extensions():
	# Mutants_Game compat patch (Cluster 3, ADR-017): upstream recognizes the generic
	# "json" extension, which makes this importer claim EVERY .json in the project
	# (catalog/*.json, tests/*.json) and flag the non-ink ones as failed imports.
	# We restrict it to the dedicated "inkjson" extension so compiled stories are named
	# "<name>.inkjson" and plain data .json files are left entirely alone. See
	# client/addons/THIRD_PARTY.md > Local modifications.
	return ["inkjson"];
