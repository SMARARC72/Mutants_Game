-- 0001_init.sql — Mutants_Game base schema (13 tables), refactored from supabase/schema.sql
-- into the Supabase CLI migration chain (TDD §5.2, D2). Faithful to schema.sql v0.1;
-- hardening (auth wiring, audit columns, enums/CHECKs, on-delete, indexes, art_assets
-- additions) lands in 0002_hardening.sql; RLS lands in 0003_rls.sql.
-- Forces are the shared language; corruption is the shared cost; essence is the shared fuel.

-- ========== SPECIES (the registry / templates) ==========
create table species (                      -- the creature registry (407 art-backed today; ~1,067 target)
...
-- indexes (the base set; 0002 replaces the creature_instances index with a partial one)
